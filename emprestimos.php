<?php
include 'config.php';
$sql = "SELECT e.id AS emprestimo_id, u.nome AS usuario, l.titulo AS livro, e.data_emprestimo, e.data_devolucao FROM Emprestimo e JOIN Usuario u ON e.id_usuario = u.id JOIN Emprestimo_Livro el ON e.id = el.id_emprestimo JOIN Livro l ON el.id_livro = l.id";
...
<head>
    ...
    <style>
        ...
        .container { max-width: 800px; margin: 50px auto; background: #fff; border-radius: 12px; box-shadow: 0 2px 12px #0001; padding: 32px 40px; }
